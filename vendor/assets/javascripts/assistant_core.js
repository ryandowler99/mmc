...
Balloon.prototype.speak = function speak (complete, text, hold) {
	...
	//assistant speaking the bubble text (tts)
	
	assistantSpeak(1, text);
};
...
Agent.prototype._startDrag = function _startDrag (e) {
    ...
    $(window).on('touchemove', this._moveHandle);
    $(window).on('mouseup', this._upHandle);
    $(window).on('touchendy', this._upHandle);
...
Agent.prototype._dragMove = function _dragMove (e) {
	...
	if(!assistantMenu){
		assistantMenu = document.getElementsByClassName('assistantMenu')[0];
		assistantMenu.style.visibility = 'visible';
	}
	//move the assistent with agent 
	assistantMenu.style.left = (x - 250);
	assistantMenu.style.top = (y + 125);
	*/
	//checkIfAssistantNearDock();
	console.log(x);
	if(x > 1300){
		alert("hide em");
	}
...
function checkIfAssistantNearDock(){
		console.log("ddddddgghh666");
		var first = $('.clippy').offset().left;
		var second = $('.sidenav').offset().left;
		var firstNum =  parseInt(first);
		var secondNum = parseInt(second);
		var distance = firstNum - secondNum;
		return distance
	}
...
function assistantSpeak(voiceIsEnabled, text){
	console.log("speaking now prop");
		if (voiceIsEnabled == 1 && text){
