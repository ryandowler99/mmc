...
var tempCount = 0; //temp store how  many times user input less than 15 on items
	...
	function addWheelValuesToForm(wheelValues){
		if(curIteration <= 0){
			alert("1" + curIteration);
			// var input = $('.fields').find('input')[curIteration];
			...
			alert("2fff");
			// var input = chosen_fields_as_arrayJS[curIteration-2] + "Field" ;
			// var label = chosen_fields_as_arrayJS[(curIteration -2 )] //get the label (starts at index 2, holds labels(field names) for wheel spins					becasue first label is present in html and because this function runs after field has been used) ./ 	
			// $('#' + input).val(theValue);
		}
		...
		if(!label){
			...
			$('.nextCategoryArrow').addClass("hidden");
			$('#titleImg_category').addClass("hidden");
			$('#wheelCycleBtn').remove();
			$('#categoriesDock').addClass("hidden");
			$('#genieCategoryShow').remove(); //to delete its chil elems too
			$('#pageIntroduceBtn').remove(); //to delete its chil elems too
		}
